...
app.get('/api/health', (_req, res) => {
  res.json({
    ok: true,
    message: 'AI Vlog Stability v13 API running',
...
app.post('/api/vlog/create', upload.fields([{ name: 'media', maxCount: 80 }]), async (req, res) => {
  ...
  try {
    ...
    const task = enqueueTask({ type: 'create_vlog', jobId, payload: { assets } });
...
app.post('/api/vlog/revise', (req, res) => {
  ...
  const task = enqueueTask({ type: 'revise_vlog', jobId, payload: { suggestion } });
...
app.listen(config.port, () => {
  console.log(`AI Vlog Stability v13 API: http://localhost:${config.port}`);
